;(function (g) {
      ...
      if (g.isPlainObject(d)) v(d)
        ...
        if (n) {
          ...
          for (e = c.attributes.length - 1; 0 <= e; e--) {
            ...
            ;-1 !== f.indexOf(':') &&
              (h.push({ name: f, value: j }), c.removeAttribute(f))
          }
          e = 0
          for (f = h.length; e < f; e++)
            (j = d.createAttribute(
              h[e].name.replace(':', '_dt_b_namespace_token_')
            )),
              (j.value = h[e].value),
              c.setAttributeNode(j)
        }
        c = t.serializeToString(d)
        n &&
          (-1 === c.indexOf('<?xml') &&
    ...
    x = function (a, b) {
      for (
        var d = b.newline
          ...
          f = b.fieldSeparator,
          g = RegExp(e, 'g'),
          h = b.escapeChar !== p ? b.escapeChar : '\\',
          j = function (a) {
            for (var b = '', c = 0, d = a.length; c < d; c++)
              0 < c && (b += f),
                (b += e ? e + ('' + a[c]).replace(g, h + e) + e : a[c])
            return b
          },
          l = b.header ? j(c.header) + d : '',
          k = b.footer && c.footer ? d + j(c.footer) : '',
          m = [],
          u = 0,
          B = c.body.length;
        u < B;
        ...
        m.push(j(c.body[u]))
      return { str: l + m.join(d) + k, rows: m.length }
    },
    s = {
      available: function () {
        return h.hasFlash()
  ...
  i.Buttons.swfPath = '//cdn.datatables.net/buttons/1.2.4/swf/flashExport.swf'
  ...
  i.ext.buttons.copyFlash = g.extend({}, s, {
  ...
  i.ext.buttons.csvFlash = g.extend({}, s, {
  ...
  i.ext.buttons.excelFlash = g.extend({}, s, {
    ...
    action: function (a, b, d, c) {
          ...
          for (var b = 0, c = a.length; b < c; b++) {
            ...
            if (!(null === a[b] || a[b] === p || '' === a[b])) {
              ...
              for (var n = 0, o = y.length; n < o; n++)
                if (
                  ((i = y[n]),
                  a[b].match && !a[b].match(/^0\d+/) && a[b].match(i.match))
                ) {
                  k = a[b].replace(/[^\d\.\-]/g, '')
                  i.fmt && (k = i.fmt(k))
                  k = m(f, 'c', {
                    attr: { r: d, s: i.style },
                    children: [m(f, 'v', { text: k })],
                  })
                  break
                }
              k ||
      ...
      g('sheets sheet', d.xl['workbook.xml']).attr('name', A(c))
      c.customizeData && c.customizeData(b)
      c.header && (i(b.header, e), g('row c', f).attr('s', '2'))
      for (var k = 0, n = b.body.length; k < n; k++) i(b.body[k], e)
      c.footer &&
        b.footer &&
        (i(b.footer, e), g('row:last c', f).attr('s', '2'))
      i = m(f, 'cols')
      g('worksheet', f).prepend(i)
      k = 0
      for (n = b.header.length; k < n; k++)
        i.appendChild(
          m(f, 'col', {
            attr: { min: k + 1, max: k + 1, width: z(b, k), customWidth: 1 },
          })
        )
    ...
    action: function (a, b, d, c) {
      ...
      r(
        ...
        JSON.stringify({
          title: q(c, !1),
          message:
            'function' == typeof c.message ? c.message(b, d, c) : c.message,
          colWidth: g.toArray(),
